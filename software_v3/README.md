...
提供完整六运动关节加独立夹爪的软件验证。Python 模拟 MCU 接收和执行协议，默认不连接物理串口。
...
JSON 可配置六轴方向、连杆向量、零位、限位、HOME。默认为 Z-Y-Y-X-Y-X 串联 6R 球腕：基座高度 120、上臂 160、前臂 140、工具长度 60 mm。几何零位 q=0 时 TCP=(360,0,120)、旋转为单位阵；HOME 单独配置。**所有尺寸和限位都是软件示例，需要按目标机构标定替换。**限位作用于命令角 q，几何角=q+zero，不等同于板端舵机 PWM 角。
...
`planning.py:Waypoint` 是任务数据，不持有状态枚举；`runtime.py:State` 为 `IDLE → BELT_RUN → BELT_STOP → PRE_GRASP → GRASP → LIFT → PLACE → RETURN → IDLE`。PLACE 管理 transit/place；RETURN 管理 retreat/home。故障为独立锁存标志，保留最后状态。任务路径点与状态枚举独立建模。
...
合成 delta 达 400 判接触，达到 600 停止闭合，异常阈值 1200 故障退出，2 s 未保持则超时。阈值均为软件示例。随后调用 float 权重做分类诊断，不驱动夹持反馈。
